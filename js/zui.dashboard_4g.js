        ...
        this.$.on('mousedown', '.resize-handle', function(e) {
                ...
                if(lastGrid != grid) {
                    if(lastGrid != grid && GroupDivSize == '12') {
                        if(0<(100*grid/12)&&(100*grid/12)<=25 && (blockSize.indexOf("4g1")>-1)){
                            grid="3";
                            balanceGrid = "3";
                            $col.attr('data-grid', grid).css('width', 25 + '%');
                            if(messagerAvaliable) dashboardMessager[dashboardMessager.isShow ? 'update' : 'show'](25+"%" );
                        }else if(25<(100*grid/12)&&(100*grid/12)<=50 && (blockSize.indexOf("4g2")>-1)){
                            grid="6";
                            balanceGrid = "6";
                            $col.attr('data-grid', grid).css('width', 50 + '%');
                            if(messagerAvaliable) dashboardMessager[dashboardMessager.isShow ? 'update' : 'show'](50+"%" );
                        }else if (50<(100*grid/12)&&(100*grid/12)<=75 && (blockSize.indexOf("4g3")>-1)){
                            grid="9";
                            balanceGrid = "9";
                    ...
                    }else if(lastGrid != grid && GroupDivSize == '9'){
                        if(0<(100*grid/12)&&(100*grid/12)<=33.3 && (blockSize.indexOf("4g1")>-1)){
                            grid="4";
                            balanceGrid = "3";
                            $col.attr('data-grid', grid).css('width', 33.3 + '%');
                            if(messagerAvaliable) dashboardMessager[dashboardMessager.isShow ? 'update' : 'show'](33.3+"%" );
                        }else if(33.3<(100*grid/12)&&(100*grid/12)<=66.6 && (blockSize.indexOf("4g2")>-1)){
                            grid="8";
                            balanceGrid = "6";
                            $col.attr('data-grid', grid).css('width', 66.6 + '%');
                            if(messagerAvaliable) dashboardMessager[dashboardMessager.isShow ? 'update' : 'show']( 66.6+"%" );
                        }else if(66.6<(100*grid/12)&&(100*grid/12)<=100 && (blockSize.indexOf("4g3")>-1)){
                            grid="12";
                            balanceGrid = "9";
                            $col.attr('data-grid', grid).css('width', 100 + '%');
                            if(messagerAvaliable) dashboardMessager[dashboardMessager.isShow ? 'update' : 'show']( 100+"%" );
                        }
                    }else if(lastGrid != grid && GroupDivSize == '6'){
                        if(0<(100*grid/12)&&(100*grid/12)<=50 && (blockSize.indexOf("4g1")>-1)){
                            grid="6";
                            balanceGrid = "3";
                            $col.attr('data-grid', grid).css('width',50 + '%');
                            if(messagerAvaliable) dashboardMessager[dashboardMessager.isShow ? 'update' : 'show'](50+"%" );
                        }else if(50<(100*grid/12)&&(100*grid/12)<=100 && (blockSize.indexOf("4g2")>-1)){
                            grid="12";
                            balanceGrid = "6";
                            $col.attr('data-grid', grid).css('width', 100 + '%');
                            if(messagerAvaliable) dashboardMessager[dashboardMessager.isShow ? 'update' : 'show']( 100+"%" );
            ...
            var mouseUp = function(event) {
                ...
                if(oldGrid !== balanceGrid){
                    ...
                    pBlockId = pBlockId[0];
                    if(balanceGrid == "3"){
                        var newBlockId = pBlockId+"_4g1";
                        if(nextShowId == undefined){
                            nextShowId="";
                        }
                        importFile(newBlockId,nextShowId,blockSizes,groupDiv);
                    }else if(balanceGrid == "6"){
                        var newBlockId = pBlockId + "_4g2";
                        if(nextShowId == undefined){
                            nextShowId="";
                        }
                        importFile(newBlockId,nextShowId,blockSizes,groupDiv);
                    }else if(balanceGrid == "9") {
                        var newBlockId = pBlockId + "_4g3";
                        if (nextShowId == undefined) {
                            nextShowId = "";
                        }
                        importFile(newBlockId, nextShowId, blockSizes, groupDiv);
